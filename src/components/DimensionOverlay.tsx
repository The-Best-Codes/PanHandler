...
export default function DimensionOverlay({ 
  ...
  // __DEV__ && console.log('✅ DimensionOverlay v4.0 loaded - Static Tetris active');
  ...
  const triggerTetrisAnimation = () => {
    __DEV__ && console.log('🎮 STATIC TETRIS v4.0 - Simple fade animation');
    setShowTetris(true);
  ...
  const doesPathSelfIntersect = (path: Array<{ x: number; y: number }>): boolean => {
    ...
    __DEV__ && console.log(`🔍 Self-intersection check: path length ${path.length}, excluding first ${exclusionZoneSize} and last ${exclusionZoneSize} segments`);
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      for (let j = i + 2; j < path.length - 1; j++) {
        ...
        if (doSegmentsIntersect(seg1Start, seg1End, seg2Start, seg2End)) {
          __DEV__ && console.log(`❌ Self-intersection detected between segment ${i} and segment ${j}`);
          return true;
        }
      }
    }
    
    __DEV__ && console.log('✅ No self-intersection detected (excluding start/end zones)');
    return false;
  };
  ...
  const saveOriginalState = (measurementId: string) => {
    ...
    if (!measurementHistory.has(measurementId)) {
      ...
      if (measurement) {
        ...
        __DEV__ && console.log('💾 Saved original state for measurement:', measurementId);
      }
    }
  };
  ...
  const recalculateMeasurement = (measurement: Measurement): Measurement => {
    ...
    if (mode === 'distance') {
      ...
      __DEV__ && console.log('⚠️ Freehand shape edited - removing area from legend (perimeter still valid)');
      return { 
        ...measurement, 
  ...
  const placePoint = (x: number, y: number) => {
    ...
    __DEV__ && console.log('🎯 Placing point:');
    __DEV__ && console.log('  Screen coords:', x, y);
    __DEV__ && console.log('  Image coords:', imageCoords.x.toFixed(1), imageCoords.y.toFixed(1));
    __DEV__ && console.log('  Current zoom:', zoomScale.toFixed(2), 'translate:', zoomTranslateX.toFixed(0), zoomTranslateY.toFixed(0));
    ...
    if (currentPoints.length + 1 < requiredPoints) {
      ...
      __DEV__ && console.log('📏 Created measurement:', {
        mode,
  ...
  useEffect(() => {
    ...
    if (zoomChanged && showLockedInAnimation) {
      __DEV__ && console.log('🚫 Pan/zoom detected - dismissing lock-in animation');
  ...
  useEffect(() => {
    ...
    if (legendHeight >= triggerHeight) {
      __DEV__ && console.log('🎮 TETRIS EASTER EGG TRIGGERED!', measurements.length, 'measurements');
  ...
  const handleClear = () => {
    ...
    if (measurements.length > 0) {
      ...
      if (measurementHistory.has(lastMeasurement.id)) {
        ...
        __DEV__ && console.log('↩️ Reverted measurement to original state:', lastMeasurement.id);
      ...
      {measurementMode && (
        <View
          ...
          onResponderGrant={(event) => {
            ...
            __DEV__ && console.log('👆 Touch started - activating cursor');
            ...
            if (mode === 'freehand') {
              ...
              freehandActivationTimerRef.current = setTimeout(() => {
                ...
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                __DEV__ && console.log('🎨 Freehand drawing activated!');
              }, 1500); // 1.5 seconds
                    ...
                    if (distance > 2) {
                      ...
                      if (prevPath.length >= 10) { // Need at least 10 points to make a meaningful loop
                        ...
                        if (distToStart < snapThresholdPixels) {
                          ...
                          if (!selfIntersects) {
                            __DEV__ && console.log('🎯 LASSO SNAP! Closing loop at', distToStart.toFixed(1), 'pixels from start');
                            ...
                            __DEV__ && console.log('⚠️ Path self-intersects - NOT snapping to start point (allowing free drawing)');
          ...
          onResponderRelease={() => {
            __DEV__ && console.log('✅ Touch released');
            ...
            if (mode === 'freehand') {
              ...
              if (freehandActivating && !isDrawingFreehand) {
                ...
                __DEV__ && console.log('⚠️ Freehand activation cancelled (released too early)');
                ...
                __DEV__ && console.log('🎨 Freehand path points captured:', freehandPath.length, 'Closed loop:', isClosedLoop);
                ...
                __DEV__ && console.log('🔍 Self-intersection check:', selfIntersects);
                __DEV__ && console.log('🔍 Closed loop state:', isClosedLoop);
                __DEV__ && console.log('🔍 Path length:', freehandPath.length);
                
                // Calculate area only if loop is closed AND doesn't self-intersect
                let area = 0;
                if (isClosedLoop && !selfIntersects && freehandPath.length >= 3) {
                  __DEV__ && console.log('✅ Conditions met for area calculation!');
                  ...
                  __DEV__ && console.log('📐 Raw area in pixels²:', area.toFixed(2));
                  
                  // Convert from pixel² to physical units²
                  const physicalArea = area / (pixelsPerUnit * pixelsPerUnit);
                  
                  __DEV__ && console.log('📐 Physical area:', physicalArea.toFixed(2), 'square units');
                  __DEV__ && console.log('📐 Calibration unit:', calibration?.unit);
                  __DEV__ && console.log('📐 Unit system:', unitSystem);
                  ...
                  __DEV__ && console.log('📐 Formatted perimeter:', perimeterStr);
                  __DEV__ && console.log('📐 Formatted area:', areaStr);
                  __DEV__ && console.log('📐 Final value string:', formattedValue);
                  ...
                  __DEV__ && console.log('📐 Created measurement:', JSON.stringify(newMeasurement, null, 2));
                  
                  setMeasurements([...measurements, newMeasurement]);
                } else {
                  __DEV__ && console.log('⚠️ Area calculation skipped - conditions not met:', {
                    isClosedLoop,
                    selfIntersects,
                  ...
                  if (isClosedLoop && selfIntersects) {
                    __DEV__ && console.log('⚠️ Closed loop detected, but path self-intersects - area calculation skipped');
                  }
                }
                
                __DEV__ && console.log('🎨 Creating freehand measurement with', freehandPath.length, 'points');
                ...
                // Success haptic
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                __DEV__ && console.log('🎨 Freehand measurement completed with', freehandPath.length, 'points');
              } else if (isDrawingFreehand) {
                // Path too short, just reset
                __DEV__ && console.log('⚠️ Path too short (', freehandPath.length, 'points), need at least 5 - discarding');
                setFreehandPath([]);
                setIsDrawingFreehand(false);
                setShowFreehandCursor(false);
                setFreehandActivating(false);
              ...
              if (tappedId) {
                ...
                if (tapDeleteState?.measurementId === tappedId && (now - tapDeleteState.lastTapTime) < TAP_TIMEOUT) {
                  ...
                  if (newCount >= 4) {
                    ...
                    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                    __DEV__ && console.log('🗑️ Measurement deleted via 4 rapid taps');
              ...
              <Pressable
                ...
                onPressIn={() => {
                  ...
                  freehandLongPressRef.current = setTimeout(() => {
                    ...
                    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                    __DEV__ && console.log('🎨 Freehand mode activated via long-press');
